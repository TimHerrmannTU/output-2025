<div id="challenges" class="dark-bg">
    <div class="wrapper cards row mobile-col gap-3">
        <div class="card gc col gap-1">
            <h3>GEOCACHING</h3>
            <img src="<?= get_template_directory_uri(); ?>/static/img/geoCaching.png"/>
            <a href="<?= get_site_url(); ?>/geo-caching"><button class="more white transparent-text">Zur Anleitung ...</button></a>
        </div>
        <div class="card cc col gap-1">
            <h3>CREATIVE CHALLENGE</h3>
            <img src="<?= get_template_directory_uri(); ?>/static/img/creativeChallenge.png"/>
            <a href="<?= get_site_url(); ?>/creative-challenge"><button class="more white transparent-text">Zur Teilnahme ...</button></a>
        </div>
        <div class="card lp col gap-1">
            <h3>LAN - PARTY</h3>
            <img src="<?= get_template_directory_uri(); ?>/static/img/lanParty.png"/>
            <a href="<?= get_site_url(); ?>/lan-party"><button class="more white transparent-text">Zur Anmeldung ...</button></a>
        </div>
    </div>
</div>